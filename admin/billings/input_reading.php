...
?>
<div class="mx-0 py-5 px-3 mx-ns-4 bg-gradient-info">
	<h3><b><?= isset($id) ? "Input Reading - ".(isset($code) ? $code : '') : "Input Reading" ?></b></h3>
</div>
<style>
	img#cimg{
      max-height: 15em;
	...
	<div class="col-lg-6 col-md-8 col-sm-11 col-xs-11">
		<div class="card rounded-0 shadow">
				...
				<div class="container-fluid">
					<div class="container-fluid">
						<form action="" id="reading-form">
							...
							<input type="hidden" id="rate" name="rate" value="<?= isset($rate) ? $rate : $_settings->info('rate') ?>"/>
							<input type="hidden" id="total" name="total" value="<?= isset($total) ? $total : '' ?>"/>
			...
			<div class="card-footer py-1 text-center">
				<button class="btn btn-info btn-sm bg-gradient-info rounded-0" form="reading-form"><i class="fa fa-save"></i> Save Reading</button>
			...
			$.ajax({
				...
				success:function(resp){
					if(typeof resp =='object' && resp.status == 'success'){
						alert_toast("Reading saved successfully",'success');
						setTimeout(function(){
